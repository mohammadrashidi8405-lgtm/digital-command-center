...
5. **DEEP MATCH** — only for jobs already scoring ≥60 on the cheap deterministic pass (§21 "deep analysis only for high-potential candidates"). Calls `brain.generate()`; with the current file-drop Brain this always comes back `pending` and is stored as advisory metadata only — it never changes the numeric score, keeping scoring auditable and reproducible without a live LLM.
